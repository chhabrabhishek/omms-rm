...
def update_release(request, form: UpdateReleaseRequest):
    ...
    for approver in list(release.approvers.all()):
        if approver.approved and Roles.Role.DevOps not in [
            ...
            with transaction.atomic():
                ...
                if len(false_branches):
                    ...
                    for item in form.release.items:
                        ...
                        release_item.release_branch = (
                            item.release_branch
                            if item.release_branch != "default_value_unique"
                            else release_item.release_branch
                        )
                        release_item.feature_number = (
                            item.feature_number
                            if item.feature_number != "default_value_unique"
                            else release_item.feature_number
                        )
                        release_item.tag = (
                            item.tag
                            if item.tag != "default_value_unique"
                            else release_item.tag
                        )
                        release_item.special_notes = (
                            item.special_notes
                            if item.special_notes != "default_value_unique"
                            else release_item.special_notes
                        )
                        if Roles.Role.DevOps in [
                            role.role for role in list(user.roles.all())
                        ]:
                            release_item.devops_notes = (
                                item.devops_notes
                                if item.devops_notes != "default_value_unique"
                                else release_item.devops_notes
                            )
                        release_item.save()
                    for item in form.release.talend_items:
                        if item.id:
                            ...
                            if (
                                item.job_name
                                or item.package_location
                                or item.feature_number
                                or item.special_notes
                            ):
                                talend_release_item.job_name = item.job_name
                                talend_release_item.package_location = (
                                    item.package_location
                                )
            ...
            talend_items = TalendReleaseItem.objects.filter(release=release)
            return {
                "ok": True,
...
def get_all_releases(request):
    try:
        releases_list = Release.objects.order_by("-start_window")[:50]
